...
describe King do
  describe "#initialize" do
    board = Board.new
    white_king = King.new(:white, [4, 0], board)
    black_king = King.new(:black, [4, 7], board)
  ...
  describe "#get_moves" do
    board = Board.new
    board.board[5][3] = King.new(:white, [5, 3], board)
    board.board[2][4] = King.new(:black, [2, 4], board)
    white_king = board.board[5][3]
    black_king = board.board[2][4]

    it "should return a move for every direction" do
      expect(white_king.get_moves).to include([6, 4],
                                                     [6, 3],
                                                     [6, 2],
                                                     [5, 4],
                                                     [5, 2],
    ...
    it "should return no moves when surrounded by friendly pieces" do
      expect(board.board[4][0].get_moves).to eq([])
    end

    it "should include enemy locations" do
      board.board[5][4] = King.new(:black, [5, 4], board)
      expect(white_king.get_moves).to include([5, 4])
    end

    it "should not include locations that are out of bounds" do
      actual = white_king.get_moves.any? { |move| move.any? {|i| i < 0 || i > 7 } }
