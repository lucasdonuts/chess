  ...
  context "getting moves" do
    describe "#vertical_moves" do
      context "moving up" do
        ...
        it "should return no moves when blocked in by friendly pieces" do
          expect(white_rook.vertical_moves(board)).to eq([])
        end

        it "should include enemy locations on it's path" do
          board.board[0][1] = nil
          expect(white_rook.vertical_moves(board)).to include([0, 6])
        end

        it "should stop adding moves when reaching a non-empty square" do
          expect(white_rook.vertical_moves(board)).not_to include([0, 7])
        end

        it "should not return moves that are out of bounds" do
          board.board[0][6] = nil
          board.board[0][7] = nil
          actual = board.board[0][0].vertical_moves(board).any? { |move| move.any? {|i| i < 0 || i > 7 } }
      ...
      context "moving down" do
        ...
        it "should return no moves when blocked in by friendly pieces" do
          expect(black_rook.vertical_moves(board)).to eq([])
        end

        it "should include enemy locations on it's path" do
          board.board[0][6] = nil
          expect(black_rook.vertical_moves(board)).to include([0, 1])
        end

        it "should stop adding moves when reaching a non-empty square" do
          expect(black_rook.vertical_moves(board)).not_to include([0, 0])
        end

        it "should not return moves that are out of bounds" do
          board.board[0][1] = nil
          board.board[0][0] = nil
          actual = black_rook.vertical_moves(board).any? { |move| move.any? {|i| i < 0 || i > 7 } }
          expect(actual).to be false
        end
      end
    end
    ...
    describe "#horizontal_moves" do
      context "moving left" do
        ...
        it "should return no moves when blocked in by friendly pieces" do
          expect(board.board[7][0].horizontal_moves(board)).to eq([])
        end

        it "should include enemy locations on it's path" do
          expect(board.board[5][3].horizontal_moves(board)).to include([1, 3])
        end

        it "should stop adding moves when reaching a non-empty square" do
          expect(board.board[5][3].horizontal_moves(board)).not_to include([0, 3])
        end

        it "should not return moves that are out of bounds" do
          board.board[1][3] = nil
          actual = board.board[5][3].horizontal_moves(board).any? { |move| move.any? {|i| i < 0 || i > 7 } }
          expect(actual).to be false
        end
      end
      ...
      context "moving right" do
        ...
        board.board[1][3] = Rook.new(:black, [1, 3])
        it "should return no moves when blocked in by friendly pieces" do
          expect(board.board[0][7].horizontal_moves(board)).to eq([])
        end

        it "should include enemy locations on it's path" do
          expect(board.board[1][3].horizontal_moves(board)).to include([5, 3])
        end

        it "should stop adding moves when reaching a non-empty square" do
          expect(board.board[1][3].horizontal_moves(board)).not_to include([6, 3])
          expect(board.board[1][3].horizontal_moves(board)).not_to include([7, 3])
        end

        it "should not return moves that are out of bounds" do
          board.board[5][3] = nil
          actual = board.board[1][3].horizontal_moves(board).any? { |move| move.any? {|i| i < 0 || i > 7 } }
          expect(actual).to be false
        end
      end
    end
