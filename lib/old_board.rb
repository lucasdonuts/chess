...
class Board
  ...
  def castle_check(piece, destination)
    return if !piece.instance_of?(King)
  ...
  def passant_vulnerable?(piece, destination)
    if piece.color == :white && piece.first_move
      return piece.location[1] == 1 && destination[1] == 3
    elsif piece.color == :black && piece.first_move
      return piece.location[1] == 6 && destination[1] == 4
    else
      false
    end
  end
  ...
  def move_pawn_2(pawn, destination)
    pawn.color == :white ? @passant_opening = [destination[0], destination[1] - 1] :
                          @passant_opening = [destination[0], destination[1] + 1]
